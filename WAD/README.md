...
- decode `FONT` glyph material/metric tables to CSV
...
- decode `AMPC` ambient-audio resource banks and 40-byte ambient emitter records
- decode `TRAK` terrain/world geometry records, vertices, triangles, collision/contact entries, OBJ surfaces, and an HTML viewer
- parse `STPC` with the executable-confirmed table/cursor layout, Section2 relocation model, and OBJ export for all decoded geometry records
...
extracted/t1l1m001/
  ...
  font/                 FONT glyph metrics and material references
  ampc/                 AMPC ambient-audio resource and emitter diagnostics
...
| `world/stpc_object_vm_diagnostics.csv` | Partial STPC object-definition VM fingerprints, including confirmed runtime flag/mode opcode counts and operand histograms, for comparing object archetypes across WADs. |
...
| `FONT` | ~70% | Parses 256 glyph records, exports material index, y-center offset, advance width, draw height, and text-width samples. | Exact semantics of inline text control bytes and uncommon non-printable glyphs. |
...
| `STPC` | ~74% | Parses the confirmed top-level geometry cursor layout, including matrix-group/skinned records and the Section2 variable-array relocation model; exports OBJ/MTL, manifest, face diagnostics, script-to-geometry refs, `0xB2` pointer candidates, and partial object-VM diagnostics; documents the core VM stack, role-pointer, load/store, debug-named high opcodes, model-bind, child-spawn, movement, DEFANIM references, route transforms, and the main `Actor340 +0xEC/+0xE8` flag writers. | Remaining high-opcode semantics, animation payload fields, Block32 semantics, and full IDE-friendly object/script names. |
...
Parts of `LNFO` are preserved for later analysis.  `AMPC`, `FONT`, `LGPC`, `WFPC`, and `SPRT` now have confirmed first-pass decoding, but several WFPC bits and the higher-level sprite/animation structures that consume SPRT slots still need more runtime analysis.
...
eng_wad/font_chunk.py     FONT glyph metric/material exports
eng_wad/ampc_chunk.py     AMPC ambient audio resource/emitter exports
...
5. Finish AMPC `pBAV`/`pQES`, SPRT high-level consumers, observed-only WFPC bits, and remaining text/font control behavior.
6. Build a single editor/viewer combining decoded textures, TRAK terrain, STPC objects/scripts, MAP placement/routes, LGHT lights, and audio emitters.
